...
function sourceLabelFromType(sourceType) {
...
function applyAddMode(mode) {
  state.addMode = mode;
  const tabWrap = document.getElementById('addModeTabs');
  if (!tabWrap) return;

  tabWrap.querySelectorAll('button[data-action="add-mode"]').forEach((button) => {
    button.classList.toggle('active', button.dataset.mode === mode);
  });

  const sections = document.querySelectorAll('#screen-add [data-add-section]');
  sections.forEach((section) => {
    const sectionMode = section.getAttribute('data-add-section');
    const visible = mode === 'search' ? ['search', 'favorites'].includes(sectionMode) : mode === sectionMode;
    section.hidden = !visible;
  });
}

async function quickLogItem(item, personId) {
  const grams = 100;
  ...
  await addEntry({
    ...
    amountGrams: grams,
    ...macros,
    micronutrients: computeEntryMicronutrients(item.nutrition, grams),
    source: sourceLabelFromType(item.sourceType),
...
const ONBOARDING_STEPS = [
  {
    ...
    body: 'Track progress with a calm, private dashboard. Calories, macros, streaks, and habits are designed for steady consistency.',
    art: '🌿'
  },
  {
    title: 'Log food your way',
    body: 'Use barcode scan, manual search, or photo workflow. Quick Add and Favorites help you log meals in seconds.',
    art: '📱'
  },
  {
    title: 'Private and offline-first',
    body: 'Your data stays on-device in IndexedDB. The app works offline and never needs cloud sync to stay useful.',
    art: '🔒'
  }
];
...
function renderOnboardingStep() {
  ...
  if (art) art.textContent = step.art;
  if (dots) {
    dots.innerHTML = ONBOARDING_STEPS.map((_, i) => `<span class="onboarding-dot ${i === state.onboardingStep ? 'active' : ''}"></span>`).join('');
  }

  const prev = document.getElementById('onboardingPrevBtn');
  const next = document.getElementById('onboardingNextBtn');
  const done = document.getElementById('onboardingDoneBtn');
  if (prev) prev.disabled = state.onboardingStep === 0;
  if (next) next.hidden = state.onboardingStep >= ONBOARDING_STEPS.length - 1;
  if (done) done.hidden = state.onboardingStep < ONBOARDING_STEPS.length - 1;
}

async function maybeShowOnboarding(personId) {
  if (!personId) return;
  const dialog = document.getElementById('onboardingDialog');
  if (!dialog || dialog.open) return;

  const seenKey = `onboardingSeen:${personId}`;
  const seen = await getMetaValue(seenKey);
  if (seen) return;

  state.onboardingPersonId = personId;
...
async function completeOnboarding() {
  const personId = state.onboardingPersonId || state.selectedPersonId;
  if (!personId) return;
  await setMetaValue(`onboardingSeen:${personId}`, true);
  const dialog = document.getElementById('onboardingDialog');
  if (dialog?.open) dialog.close();
}

async function updateInstallBanner() {
  const banner = document.getElementById('installBanner');
  if (!banner) return;
  const dismissed = await getMetaValue('installBannerDismissed');
  banner.hidden = Boolean(dismissed) || isStandaloneInstalled();
}
...
async function computeLoggingStreak(personId, endDate) {
  const maxDays = 90;
  const startDate = shiftIsoDate(endDate, -(maxDays - 1));
  const entries = await getEntriesForPersonDateRange(personId, startDate, endDate);
  const loggedDates = new Set(entries.map((entry) => entry.date));

  let streak = 0;
  for (let i = 0; i < maxDays; i += 1) {
    const currentDate = shiftIsoDate(endDate, -i);
    if (!loggedDates.has(currentDate)) break;
    streak += 1;
  }
  return streak;
}
...
function sectionItems(personId) {
  const categoryById = new Map(genericFoods.map((food) => [food.id, food.category]));

  const favorites = (state.favoritesByPerson[personId] || []).map((item) => ({
    ...item,
    isGeneric: item.sourceType === 'generic',
    groupLabel: 'Favorite',
    category: item.category || categoryById.get(item.foodId) || null,
...
async function loadAndRender() {
  ...
  applyAddMode(state.addMode);
  await updateInstallBanner();

  const person = state.persons.find((p) => p.id === state.selectedPersonId);
  if (person) {
    await maybeShowOnboarding(person.id);
    const streakDays = await computeLoggingStreak(person.id, state.selectedDate);
...
async function logActiveFood() {
  ...
  const source = sourceLabelFromType(active.sourceType);
...
async function handleAddSuggestionClick(e) {
  ...
  if (action === 'quick-log') {
    try {
      await quickLogItem(item, personId);
      await loadAndRender();
    } catch (error) {
      console.error(error);
      showAddStatus('Quick add failed.');
    }
    return;
  }
...
function wireEvents() {
  ...
  const on = (id, eventName, handler) => {
    ...
    const existing = boundNodes.get(node) || new Set();
    if (existing.has(eventName)) return node;
    existing.add(eventName);
    boundNodes.set(node, existing);
    node.addEventListener(eventName, handler);
    return node;
  };

  on('personsList', 'click', (e) => {
    const button = e.target.closest('#goToSettingsFromPersons');
    if (!button) return;

    const settingsTab = document.querySelector('.tab[data-route="settings"]');
  ...
  on('addModeTabs', 'click', (e) => {
    const button = e.target.closest('button[data-action="add-mode"]');
    if (!button) return;
    const mode = button.dataset.mode || 'search';
    if (mode === 'scan') {
      const scanTab = document.querySelector('.tab[data-route="scan"]');
      if (scanTab) scanTab.click();
      return;
    }
    applyAddMode(mode);
    if (mode === 'quick') document.getElementById('customName')?.focus();
    if (mode === 'search') document.getElementById('foodSearchInput')?.focus();
  });

  on('dashboardSummary', 'click', async (e) => {
    const button = e.target.closest('button[data-action="macro-view"]');
    if (!button) return;
    state.dashboardMacroView = button.dataset.view || 'consumed';
    await loadAndRender();
  });
  ...
  on('onboardingSkipBtn', 'click', completeOnboarding);
  on('onboardingDoneBtn', 'click', completeOnboarding);
  on('onboardingPrevBtn', 'click', () => {
  ...
  on('dismissInstallBannerBtn', 'click', async () => {
    await setMetaValue('installBannerDismissed', true);
    await updateInstallBanner();
  });

  on('openInstallFromBannerBtn', 'click', () => {
    const installHintBtn = document.getElementById('installHintBtn');
    if (installHintBtn) installHintBtn.click();
  });
