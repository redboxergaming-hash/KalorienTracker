...
export function renderPersonsList(persons, todayStatsByPerson = {}) {
  ...
  if (!persons.length) {
    wrap.innerHTML = `
      <article class="card">
        <h3>Add a person</h3>
        <p class="muted">No persons yet. Add one to start tracking.</p>
        <button type="button" id="goToSettingsFromPersons">Add person in Settings</button>
      </article>`;
    return;
  }
...
export function renderPersonPicker(persons, selectedId) {
  const personPicker = el('personPicker');
  const addPersonPicker = el('addPersonPicker');
  const analyticsPicker = el('analyticsPersonPicker');
  const nutritionPicker = el('nutritionPersonPicker');

  if (!personPicker || !addPersonPicker) return;
  ...
  personPicker.innerHTML = html;
  addPersonPicker.innerHTML = html;
...
export function renderDashboard(person, date, entries, options = {}) {
  ...
  const entriesContainer = el('entriesTableContainer');
  if (!summary || !entriesContainer) return;
...
export function renderDashboardEmpty() {
  const summary = el('dashboardSummary');
  const entriesContainer = el('entriesTableContainer');
  if (summary) summary.innerHTML = '<p class="muted">No persons available. Add a person in Settings.</p>';
